...
返回值：
	...
	0:	成功
*/
int MyPort::OpenPort(char *strPort)
{
	if(m_hPort)
	{
		CloseHandle(m_hPort);
...
*********/
int MyPort::ReadPort(unsigned char *InBuff,int BuffLen)
{
	if(m_hPort==NULL)
		return -2;
...
*****************************/
int MyPort::WritePort(unsigned char *OutBuff, int BuffLen)
{
	if(m_hPort==NULL)
		return -2;
	COMMTIMEOUTS timeouts;
