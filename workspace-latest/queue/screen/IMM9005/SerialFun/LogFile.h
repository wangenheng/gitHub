...
protected:
	...
	virtual void WriteLog( LPCVOID lpBuffer, DWORD dwLength)//写日志, 可以扩展修改
	{
		...
		{
			time(&now);
			strftime(temp, 25, "[%Y-%m-%d %H:%M:%S]\t", localtime(&now));
			WriteFile(_hFile, temp, 22, &dwWriteLength, NULL);
	...
	void Log(LPCVOID lpBuffer, DWORD dwLength)
	{
		...
		__try
		{
			...
			if(strcmp(_szLastDate, temp) != 0)//更换文件名
			{
				if(_szFileName)
					delete []_szFileName;
				_szFileName = new char[strlen(_szPath) + strlen(temp) + 6];
