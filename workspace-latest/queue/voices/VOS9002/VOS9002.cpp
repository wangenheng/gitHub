...
Implement_DynCreate(VOS9002,VoicesAdapter)

int VOS9002::VoiceDevPlaye( int readMode, char *custmerNum, int windowsNum, char *informatin )
{
	std::cout << "播放港语" << std::endl;
...
//AddFile7.wav -> 号到
//AddFile8.wav -> 号窗口
void VOS9002::PlayVoices( char *custmerNum, int windowsNum )
{
	int  i = 0;
	int  rows = 0;
	char wavFileName[32];
...
void inline VOS9002::initWavFileArray(char arrayname[][128], int rows)
{
...
// 预处理:阅读顾客编号
void inline VOS9002::fillReadCusNum( char arrayname[][128], char *custmerNum )
{
	int i;
	for (i=0; i<strlen(custmerNum); i++) {
...
//预处理:阅读窗口号
void inline VOS9002::fillReadWinNum(char ch_num[][128], int windowsNum, int *rows)
{
	int j=0;
	int index;
