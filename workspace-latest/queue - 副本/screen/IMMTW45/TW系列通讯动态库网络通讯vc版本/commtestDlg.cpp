...
void CCommtestDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_DESCPORT, m_DescPort);
...
{ 
    ...
    ret=SendData(Sockid,csAddr.GetBuffer(0),atoi(m_DescPort),buff,sizeof(buff)/sizeof(byte),str.GetLength());
    if(ret<0) 
...
void CCommtestDlg::OnOpenscreen() 
{
    ...
    CString   csAddr;   
    csAddr.Format("%d.%d.%d.%d",nField[0],nField[1],nField[2],nField[3]);
    ret=NetShut(atoi(m_LocalPort),csAddr.GetBuffer(0),1);
    itoa(ret,buffer,10);
    CWnd::SetWindowText(buffer);	
}
...
void CCommtestDlg::OnClosescreen() 
{
    ...
    CString   csAddr;   
    csAddr.Format("%d.%d.%d.%d",nField[0],nField[1],nField[2],nField[3]);
    ret=NetShut(atoi(m_LocalPort),csAddr.GetBuffer(0),0);
    itoa(ret,buffer,10);
    CWnd::SetWindowText(buffer);	
}
...
void CCommtestDlg::OnSendpic() 
{
    ...
    CString   csAddr;   
    csAddr.Format( "%d.%d.%d.%d", nField[0], nField[1], nField[2], nField[3] );
    ret = NetSendPicData( atoi(m_LocalPort), csAddr.GetBuffer(0), atoi(m_PicNo), hdc, atoi(m_Width), atoi(m_Height), 2, 2 );
    itoa( ret, buffer, 10 );
    CWnd::SetWindowText( buffer );	
}
...
void CCommtestDlg::OnSendplan()
{
    ...
    CString   csAddr;   
    csAddr.Format("%d.%d.%d.%d",nField[0],nField[1],nField[2],nField[3]);
    ret=NetListSend(atoi(m_LocalPort),csAddr.GetBuffer(0),sendbuff,sizeof(sendbuff)/sizeof(byte),4);
    itoa(ret,buffer,10);
    CWnd::SetWindowText(buffer); 
}
...
void CCommtestDlg::OnIntercut()
{
    ...
    CString   csAddr;   
    csAddr.Format( "%d.%d.%d.%d", nField[0], nField[1], nField[2], nField[3] );
    ret = NetSendPicOnce( atoi(m_LocalPort), csAddr.GetBuffer(0), hdc, atoi(m_Width), \
		atoi(m_Height), 2, 2, showmode, showspeed, showtime );
    itoa( ret, buffer, 10 );
    CWnd::SetWindowText( buffer );
}
...
void CCommtestDlg::OnGenedata()
{
	...
	{
		iX = atoi(m_TextLeft);
		iY = (atoi(m_TextHeight)-atoi(m_FontSize)-atoi(m_FontSize)/2)/2;
	}
	else if (mode == 2)
	{
		iX = atoi(m_TextLeft);
		iY = atoi(m_TextHeight)-atoi(m_FontSize)-atoi(m_FontSize)/2-atoi(m_TextTop);
	}

	DrawRectString( GDC, str2.GetBuffer(0), m_FontName.GetBuffer(0), atoi(m_FontSize), j+1, \
		iX, iY, atoi(m_TextWidth), atoi(m_TextHeight) );

	// 水平左对齐，垂直上顶
	/*
    DrawRectString( GDC, str2.GetBuffer(0), m_FontName.GetBuffer(0), atoi(m_FontSize), j+1, \
		atoi(m_TextLeft), atoi(m_TextTop), atoi(m_TextWidth), atoi(m_TextHeight) );

		*/
	/*
	// 水平左对齐，垂直居中
	DrawRectString( GDC, str2.GetBuffer(0), m_FontName.GetBuffer(0), atoi(m_FontSize), j+1, \
		atoi(m_TextLeft), (atoi(m_TextHeight)-atoi(m_FontSize))/2, atoi(m_TextWidth), atoi(m_TextHeight) );
	

	// 水平左对齐，垂直下底
	DrawRectString( GDC, str2.GetBuffer(0), m_FontName.GetBuffer(0), atoi(m_FontSize), j+1, \
		atoi(m_TextLeft), atoi(m_TextHeight)-atoi(m_FontSize)-atoi(m_FontSize)/2-atoi(m_TextTop), atoi(m_TextWidth), atoi(m_TextHeight) );
		
	*/
	/*
	CString strInfo;
	strInfo.Format( "Height=%d FontSize=%d", atoi(m_TextHeight), atoi(m_FontSize) );
	...
	*/
	...
	/*
	DrawRectString( GDC, str2.GetBuffer(0), m_FontName.GetBuffer(0), atoi(m_FontSize), j+1, \
		atoi(m_TextWidth)/2, atoi(m_TextTop), atoi(m_TextWidth), atoi(m_TextHeight) );
	*/
	// 右对齐
	/*
	DrawRectString( GDC, str2.GetBuffer(0), m_FontName.GetBuffer(0), atoi(m_FontSize), j+1, \
		atoi(m_TextWidth), atoi(m_TextTop), atoi(m_TextWidth), atoi(m_TextHeight) );
	*/
/*
...
void CCommtestDlg::OnSendgenepic()
{
    ...
    CString   csAddr;   
    csAddr.Format("%d.%d.%d.%d",nField[0],nField[1],nField[2],nField[3]);
    ret=NetSendPicData(atoi(m_LocalPort),csAddr.GetBuffer(0),atoi(m_PicNo),GDC,atoi(m_Width),atoi(m_Height),2,2);
    itoa(ret,buffer,10);
    CWnd::SetWindowText(buffer);	
}
